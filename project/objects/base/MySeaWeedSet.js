...
export class MySeaWeedSet{
    ...
    constructor(scene, no_clusters, minRadius, maxRadius, nestXPos, nestYPos, nestRadius){
        this.scene = scene;
    ...
    initObjects(){
        ...
        for (var i = 0; i < this.no_clusters; i++){
            ...
            while (true){
                ...
                if (Math.sqrt(Math.pow(swCoords[0] - this.nestXPos, 2) + Math.pow(swCoords[2] - this.nestYPos, 2)) > this.nestRadius + this.maxRadius +1.0){
                    break;
                }
            }   
